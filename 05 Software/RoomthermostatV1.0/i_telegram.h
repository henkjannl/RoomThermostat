...
String secondsToDuration(float timeInSecs) {
  ...
  if( timeInSecs > SECS_PER_YEAR )   { n = int( timeInSecs / SECS_PER_YEAR );   result += String(n)+" years ";   timeInSecs-= n*SECS_PER_YEAR;   }
  if( timeInSecs > SECS_PER_WEEK )   { n = int( timeInSecs / SECS_PER_WEEK );   result += String(n)+" weeks ";   timeInSecs-= n*SECS_PER_WEEK;   }
  if( timeInSecs > SECS_PER_DAY )    { n = int( timeInSecs / SECS_PER_DAY );    result += String(n)+" days ";    timeInSecs-= n*SECS_PER_DAY;    }
  if( timeInSecs > SECS_PER_HOUR )   { n = int( timeInSecs / SECS_PER_HOUR );   result += String(n)+" hours ";   timeInSecs-= n*SECS_PER_HOUR;   }
  if( timeInSecs > SECS_PER_MINUTE ) { n = int( timeInSecs / SECS_PER_MINUTE ); result += String(n)+" minutes "; timeInSecs-= n*SECS_PER_MINUTE; }
  if( timeInSecs > 1 )               { n = int( timeInSecs );                   result += String(n)+" seconds";                            }
...
void TelegramChat::updateLabels() {
  ...
  if( xSemaphoreTake( controllerDataSemaphore, 1000/portTICK_PERIOD_MS ) == pdPASS ) {
    //commands[cmdSetpointLower            ].label = temperature(controllerData->overrideSetpoint-0.5);
    //commands[cmdSetpointHigher           ].label = temperature(controllerData->overrideSetpoint+0.5);
    ...
    commands[cmdHighTemperatureDown      ].label = temperature(controllerData->highTemp-0.5);
    commands[cmdHighTemperatureUp        ].label = temperature(controllerData->highTemp+0.5);
    commands[cmdLowTemperatureDown       ].label = temperature(controllerData->lowTemp -0.5);
    commands[cmdLowTemperatureUp         ].label = temperature(controllerData->lowTemp +0.5);
...
String TelegramChat::keyboard() {
  ...
  switch(screen) {
    ...
    case scnSettingsMain:
      ...
      //result+= "["+ btnInline(cmdReportBoiler)     + "," + btnInline(cmdReportTiming)     + "," + btnInline(cmdReportDebug) + "],"; 
      result+= "["+ btnInline(cmdReportBoiler)     + "," + btnInline(cmdReportDebug)      + "],"; 
      result+= "["+ btnInline(cmdMenuMain)                                                + "]]";
    break;
...
String TelegramChat::handleMessage(String & message) {
  ...
  if(message=="/start") {
    ...
    if( xSemaphoreTake( controllerDataSemaphore, 1000/portTICK_PERIOD_MS ) == pdPASS ) {
      ...
      response += " - Pressure: "+String(controllerData->boilerPressure,1) + "\n";
...
void TelegramChat::handleCallback(int messageID, String & callback) {
  ...
  switch(command) {
    ...
    default: 
      ...
      Serial.printf("Sending Telegram > Telegram [%s] keyboardMode: %s\n", commandLabels[command].c_str(), keyboardMode ? "Y" : "N");
...
String TelegramChat::displayResponse(userEventMessage_t & message) {
  ...
  if( xSemaphoreTake( controllerDataSemaphore, 1000/portTICK_PERIOD_MS ) == pdPASS ) {
    ...
    switch (message.command)  {
      ...
      case cmdSetpointLower: response= String("The setpoint temperature is lowered to ") +temperature(controllerData->temperatureSetpoint)+"\n"; break;
      case cmdSetpointAuto: response="The setpoint is set back to automatic\n"; break;
      case cmdSetpointHigher: response= String("The setpoint temperature is increased to ") +temperature(controllerData->temperatureSetpoint)+"\n"; break;
      ...
      case cmdUpdateStatus: 
        response+= "Boiler percentage " + String(100*controllerData->boilerPercentage, 0) + "%, boiler temperature " + temperature(controllerData->actualBoilerTemperature)+"\n";
        break; 
      ...
      case cmdReportTiming: 
        response = String("\nTiming report:\n");
        response+= controllerData->logBusyTime.report();
        break;
      ...
      case cmdReportDebug:
        ...
        response=  String("*Software version: ") + VERSION + "*\n";
        response+= String("- Time since reboot: ") + secondsToDuration(t1/1e6) + "\n"; 
        response+= String("- Temperature measurement successful: ") + String(controllerData->temperatureMeasurementOK ? "Y" : "N") + "\n"; 
        response+= String("- Total free bytes in the heap: ") + String(heapInfo.total_free_bytes) + "\n"; 
        /*
        response+= String("- Total bytes allocated to data in the heap: ") + String(heapInfo.total_allocated_bytes) + "\n"; 
        response+= String("- Size of the largest free block in the heap: ") + String(heapInfo.largest_free_block) + "\n"; 
        response+= String("- Lifetime minimum free heap size: ") + String(heapInfo.minimum_free_bytes) + "\n"; 
        response+= String("- Number of blocks allocated in the heap: ") + String(heapInfo.allocated_blocks) + "\n"; 
        response+= String("- Number of free blocks in the heap: ") + String(heapInfo.free_blocks) + "\n"; 
        response+= String("- Total number of blocks in the heap: ") + String(heapInfo.total_blocks) + "\n"; 
    ...
    if(controllerData->hotWaterActive) response+=String(EMOTICON_SHOWER);
    response+= "*";
...
void TelegramHandler::handleNewMessages(int numNewMessages){
  ...
  for (int i = 0; i < numNewMessages; i++)
  {
    ...
    if (listOfChats.find(chatID) == listOfChats.end()) {
      ...
      for (auto const& chat : listOfChats) {
        Serial.printf("ID: %s [ID: %s Name: %s]\n", chat.first.c_str(), chat.second.chatID.c_str(), chat.second.userName.c_str() );
      }
    }

    listOfChats[chatID].chatID = chatID;
    listOfChats[chatID].userName = fromName;
    ...
    if(bot->messages[i].type=="message") {
      ...
      Serial.printf("Handle callback: chatID: %s  msgID: %d  txt:%s\n", chatID.c_str(), messageID, bot->messages[i].text.c_str());
...
void TelegramHandler::handleQueueCommand(userEventMessage_t & message) {
  ...
  Serial.printf("TelegramHandler::handleQueueCommand() for chatID: %s MessageID: %d\n", chatID, messageID);
...
void checkTelegramIfNeeded() {
  ...
  if ( !keyboardMode and (millis() - lastTimeTelegramPolled > TELEGRAM_INTERVAL) ) {
