...
/* ==============================================================================
   MenuGroup_t
   Holds a list of  menuItem_t together and stores which of the items is selected
   ============================================================================== */
...
void Menu::buttonSelect() {
  ...
  switch ( menuStructure[currentMenu].command() ) {
    ...
    case cmHighTempDec:
      if( (data->highTemp>3) and (data->highTemp-data->lowTemp>0.5) ) {
        data->highTemp-=0.5;
        drawTemperatureSubTitle();
      }
    break;
    ...
    case cmLowTempInc:
      if( (data->lowTemp<25) and (data->highTemp-data->lowTemp>0.5)) {
        data->lowTemp+=0.5;
        drawTemperatureSubTitle();
      }
    break;
...
void Menu::drawHomeScreenBackground() {
  ...
  tft->setTextColor(CLR_MEASURED_TEMP, CLR_CIRCLE_OUTLINE);
...
void Menu::periodicScreenUpdate() {
  // Redraw date and time four times a minute
...
void Menu::drawMeasuredTemperature() {
  ...
  sprite->fillSprite(CLR_CIRCLE_OUTLINE);    
  sprite->setTextDatum(TR_DATUM);
  sprite->loadFont(FONT_TEMP_MEASURED); 
  sprite->setTextColor(CLR_MEASURED_TEMP, CLR_CIRCLE_OUTLINE);
...
void Menu::drawDateAndTime() {
  ...
  sprite->drawString(buffer, 0, 16);
  sprite->pushSprite(19, 53);
  sprite->unloadFont();
  sprite->deleteSprite();
};
...
void Menu::drawTemperatureSetpoint() {
  ...
  Serial.printf("Temperature setpoint font height: %d\n", sprite->fontHeight());
...
void Menu::drawBoiler() {
  ...
  drawJpeg(tft, ICONS[data->showerRunning ? iconShowerOn : iconShowerOff].c_str(), 43, 104);
