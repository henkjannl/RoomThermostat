...
#define FONT_LIGHT  "Noway_Light24"
...
void setup(void) {
  ...
  if (SPIFFS.exists("/Noway_Light24.vlw")  == false) font_missing = true;
  if (SPIFFS.exists("/Noway_Regular24.vlw")== false) font_missing = true;
  if (SPIFFS.exists("/Noway_Medium24.vlw") == false) font_missing = true;
...
void loop() {
  ...
  tft.loadFont(FONT_LIGHT); // Must load the font first

  Serial.printf("Noway Light: %s w=%d h=%d\n", "XXX", tft.textWidth("XXX"), tft.fontHeight());

  tft.println("Noway Light"); // println moves cursor down for a new line
  ...
  tft.loadFont(FONT_NORMAL); // Load another different font

  Serial.printf("Noway Regular: %s w=%d h=%d\n", "XXX", tft.textWidth("XXX"), tft.fontHeight());
  ...
  // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
  // Medium font text wrapping
  // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

  tft.loadFont(FONT_MEDIUM); // Load another different font

  Serial.printf("Noway Medium: %s w=%d h=%d\n", "XXX", tft.textWidth("XXX"), tft.fontHeight());
