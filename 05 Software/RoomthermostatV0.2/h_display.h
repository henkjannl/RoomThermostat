...
class Display {
  public:
    Display(TFT_eSPI & tft, ControllerData_t & controllerData);
    void begin();
  ...
  protected:
    ...
    screen_t currentScreen     = scnHome;
    screen_t previousScreen    = scnSettingsSensorOffset;
    command_t selectedMenuItem = cmdMenuMain;
};

Display::Display(TFT_eSPI & tft, ControllerData_t & controllerData ) { 
...
void Display::processMessage(userEventMessage_t message) {
  ...
  Serial.printf("Display::processMessage() screen: %s command: %s\n", screenTitle[message.screen].c_str(), commandLabels[message.selectedMenuItem].c_str() );

  if( message.sender != sndTelegram) {
    // Telegram can send a cmdUpdateScreen command but does not know which screen is selected
    currentScreen    = message.screen;
    selectedMenuItem = message.selectedMenuItem;
  }

  // If the backlight requests flipping to the home screen, the display should not switch on the backlight in return
  if( message.command != cmdMenuHome) {
  ...
  if(message.command==cmdBoilerSending) {
    ...
    if( currentScreen==scnHome ) drawJpeg(tft, ICONS[iconBoilerTransmit].c_str(), 47, 166);  
    return;
  }

  // Redraw background if screen is changed
  if(currentScreen != previousScreen) {
    if( currentScreen==scnHome ) {
...
void Display::displayMenuScreen(userEventMessage_t message) {
  ...
  Serial.printf("Display::displayMenuScreen() %s > %s\n", screenTitle[currentScreen].c_str(), commandLabels[selectedMenuItem].c_str() );
  ...
  sprite.drawString(screenTitle[currentScreen].c_str(), 2, 2);
  ...
  int y2 = y;
  int firstItem=0;
  for (auto& menuItem : SCREENS[currentScreen] ) {
    y2+=sprite.height();   
    if(y2>240) firstItem++; 
...
Display display(tft, controllerData); // Display

void checkDisplayQueue( void *pvParameters ) {
