...
/*
  ...
  2.0.0 Switch to PlatformIO
        Removed over the air updates
        Changed filesystem from SPIFFS to LittleFS
        Changed storing settings to NVS

  TO DO:
    Potential improvements:
    * use MillisecTimer where possible
...
#define WATCHDOG_TIMEOUT 3*60  // 3 minute watchdog
...
void setup() {
  ...
  if (!LittleFS.begin(
        ...
        /*basePath=*/"",
        /*maxOpenFiles=*/10,
        /*partitionLabel=*/"littlefs"  // <- must match partition label
      )) {
    delay(300);
  ...
  controllerData.loadConfig(LittleFS, CONFIG_FILE);
  Serial.printf("Bot API '%s'\n", controllerData.botToken.c_str());

  Serial.println("Loading controller settings");
  controllerData.loadSettingsNVS();
  ...
  // WiFi.setTxPower(WIFI_POWER_8_5dBm); // default is 19.5 dBm
  // WiFi.setTxPower(WIFI_POWER_2dBm); // default is 19.5 dBm
  // WiFi.setTxPower(WIFI_POWER_MINUS_1dBm); // default is 19.5 dBm
...
void loop() {
  ...
  userEventMessage_t message;
  static MilliSecTimer settingsTimer(60*1000); // Save settings every minute if changed

  // ArduinoOTA.handle();

  // If controller settings were changed, autosave the settings every minute
  if(settingsTimer.lapsed()) {
    // lastSettingsSave = millis();
    if( controllerData.settingsChanged ) {
      disableKeyboard(); // Really necessary!
      // controllerData.saveSettings(LittleFS, SETTINGS_TEMP, SETTINGS_FILE);
      controllerData.saveSettingsNVS();
      enableKeyboard();
      Serial.println("Autosaved settings");
    }
  }
